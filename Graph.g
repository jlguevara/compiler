...
@members
{
    ...
    private HashMap<String, String> currentRegisters;
...
translate
   :  ^(PROGRAM t=types d=declarations[globalTable] f=functions[globalTable])
         {
            Type main = globalTable.get("main");
            if (main == null) {
                System.out.println("missing main function");
                System.exit(1);
            }
            if (((FunType)main).params.size() != 0) {
...
loop[HashMap<String, Type> scope]
   :  ^(ast=WHILE e=expression[scope] b=block[scope] expression[scope])
   ;
         ...
         lft=expression[localScope] rht=expression[localScope])
      {
        ...
        if (op.equals("==") || op.equals("!=")) {
            if ((leftType instanceof StructType && 
                ...
                System.out.println("Line " + $ast.line + 
                    ": structs can only be compared to null"); 
                System.exit(1); 
            } 
            if ((leftType instanceof NullType && 
                    !(rightType instanceof StructType)) ||
                    (rightType instanceof NullType &&
                    !(leftType instanceof StructType))) {
                System.out.println("Line " + $ast.line + 
                    ": nulls can only be compared to structs"); 
                System.exit(1); 
            } 
